...
class AIMenu(BaseMenu):
    ...
    def setup_buttons(self):
        ...
        self.buttons['restar_config_ai'] = pygame_gui.elements.UIButton(relative_rect=Rect(self.WIDTH-120, 280, 110, 40),text="Press",object_id="#button_on",manager=self.interface.manager,command=lambda:self.more_options([lambda:self.config.config(AI=True),self.config.save_config,lambda:self.change_mains(8,fade=False)]))
        ...
        self.buttons['type_training_ai'] = pygame_gui.elements.UIButton(relative_rect=Rect(self.WIDTH-120, 200, 110, 40),text="Change",object_id="#button_on",manager=self.interface.manager,command=lambda:self.more_options([lambda:self.change_mains(8,fade=False)]))
        self.buttons['type_model_ai'] = pygame_gui.elements.UIButton(relative_rect=Rect(self.WIDTH-120, 240, 110, 40),text="Change",object_id="#button_on",manager=self.interface.manager,command=lambda:self.more_options([lambda:self.change_mains(8,fade=False)]))
    def render(self):
