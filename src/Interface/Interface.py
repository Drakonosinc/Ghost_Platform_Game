...
class interface(load_elements):
    ...
    def menu_AI(self):
        if self.main==8:
            ...
            restar_config_ai=pygame_gui.elements.UIButton(relative_rect=Rect(self.WIDTH-120, 200, 110, 50),text="Press",object_id="#button_on",manager=self.manager,command=lambda:self.more_options(command=lambda:self.config.config(AI=True),command2=self.config.save_config,command3=lambda:self.change_mains(8,fade=False)))
