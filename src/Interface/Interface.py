...
class interface(load_elements):
    ...
    def keys_menu(self):
        ...
        restar_config_key=pygame_gui.elements.UIButton(relative_rect=Rect(360, 300, 80, 50),text="Press",object_id="#button_on",manager=self.manager,command=lambda:self.more_options(command=lambda:self.config.config(keys=True),command2=self.config.save_config,command3=lambda:self.change_mains(6,fade=False)))
        self.option_button=pygame_gui.elements.UIButton(relative_rect=Rect(10, self.HEIGHT-50, 100, 50),text='Back',manager=self.manager)
    ...
    def menu_AI(self):
        ...
        type_training_ai=pygame_gui.elements.UIButton(relative_rect=Rect(self.WIDTH-120, 200, 110, 50),text="Press",object_id="#button_on",manager=self.manager,command=lambda:self.more_options(command=lambda:self.config.config(AI=True),command2=self.config.save_config,command3=lambda:self.change_mains(8,fade=False)))
        self.screen.blit(self.font3_5.render(f"Type Model", True, "White"),(10,240))
        type_model_ai=pygame_gui.elements.UIButton(relative_rect=Rect(self.WIDTH-120, 240, 110, 50),text="Press",object_id="#button_on",manager=self.manager,command=lambda:self.more_options(command=lambda:self.config.config(AI=True),command2=self.config.save_config,command3=lambda:self.change_mains(8,fade=False)))
        ...
        self.active_buttons.extend([play,save,back_game_menu,increase_generation,decrease_generation,increase_population,decrease_population,increase_try_for_ai,decrease_try_for_ai,restar_config_ai,type_training_ai])
