...
class interface(load_elements):
    ...
    def menu_AI(self):
        ...
        type_training_ai=pygame_gui.elements.UIButton(relative_rect=Rect(self.WIDTH-120, 200, 110, 50),text="Press",object_id="#button_on",manager=self.manager,command=lambda:self.more_options(command=self.config.save_config,command3=lambda:self.change_mains(8,fade=False)))
        self.screen.blit(self.font3_5.render(f"Type Model", True, "White"),(10,240))
        type_model_ai=pygame_gui.elements.UIButton(relative_rect=Rect(self.WIDTH-120, 240, 110, 50),text="Press",object_id="#button_on",manager=self.manager,command=lambda:self.more_options(command=self.config.save_config,command3=lambda:self.change_mains(8,fade=False)))
